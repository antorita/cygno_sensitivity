...
./runfit
```
The output will be a **ROOT file containing the distribution of $\mu_b$ (background) and $\mu_s$ (signal).

Note: You may need to create the results folder (where the output is saved) before running the code.

To plot the results and evalualte the **90% Confidence Interval**, run:
```
python3 plot.py
```
